...
public class Movimiento {
    ...
    public static Table cargarMedicinas(Table tabla, Integer tipo, Date fecha_ini, Date fecha_fin, Animal animal, Corral corral) {
        ...
        String consulta = ""
                ...
                + "         DATE_FORMAT(ma.fecha, '%Y/%m/%d') fecha, "
                + "         c.nombre, "
                + "         round(ma.dosis,2) dosis , round(m.costo,2) Costo_unitario, round(ma.dosis * m.costo_unitario ,2) importe "
                + "FROM     medicina_animal ma, animal a, medicina m, corral_animal ca, corral c "
